...
// 在每日排程時間掃描指定目錄，確認是否存在包含前一日日期（YYYY-DD-MM）的檔案，
// 無論結果如何均寄送郵件通知相關人員。
package filecheck
...
// FileDateFormat 是在檔案名稱中搜尋日期時使用的 Go 時間格式，對應 YYYY-DD-MM。
// 例如 2026-04-03 會格式化為 "2026-03-04"（年-日-月，罕見但為需求格式）。
const FileDateFormat = "2006-02-01"

// ScanForDate 掃描 dir 目錄，找出名稱中包含 date（YYYY-DD-MM 格式）字串的檔案。
...
// 確保「有無皆寄」的行為得以實現。
//
// date 為被掃描的前一日日期；datePattern 為在目錄中搜尋的格式（YYYY-DD-MM）。
...
datePattern := date.Format(FileDateFormat) // YYYY-DD-MM
...
sb.WriteString(fmt.Sprintf("搜尋格式：YYYY-DD-MM（本次搜尋：%s）\n\n", datePattern))
